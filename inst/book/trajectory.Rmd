...
stopifnot(sum(tab[c("1,2,3,4", "1", "2", "3")]) > 0.95)
```
...
```{r nest-1-simple-down, fig.asp=3, fig.width=6, fig.cap="Expression of the top 10 genes that decrease in expression with increasing pseudotime along the second path in the MST of the Nestorowa dataset. Each point represents a cell that is mapped to this path and is colored by the assigned cluster."}
...
```{r nest-1-simple-up, fig.asp=3, fig.width=6, fig.cap="Expression of the top 10 genes that increase in expression with increasing pseudotime along the second path in the MST of the Nestorowa dataset. Each point represents a cell that is mapped to this path and is colored by the assigned cluster."}
...
on.second.path <- !is.na(sce.nest2$TSCAN.first)
...
```{r, echo=FALSE}
stopifnot(igraph::degree(mst, starter) == 4)
```
...
line.data.sub <- line.data[grepl("^2--", line.data$edge) | grepl("--2$", line.data$edge),]
